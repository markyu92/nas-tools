...
_TRANSFORMS: dict[str, Callable] = {
    ...
    "timestamp_to_date": lambda val: datetime.datetime.fromtimestamp(
        val, pytz.timezone(os.environ.get("TZ", "Asia/Shanghai"))
...
def _resolve_jinja(text: str, fields: dict) -> str:
    ...
    if cond_match:
        ...
        replacement = re.sub(r"\{\{\s*fields\[[\"'](\w+)[\"']\]\s*\}\}", lambda m: str(fields.get(m.group(1), "")), replacement)
