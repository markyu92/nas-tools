...
class TorrentSpider(feapder.AirSpider):
    ...
    def start_requests(self):
        ...
        if len(paths) == 1:
            ...
            for path in paths:
                if (
                    path.get("type") == "all"
                    and not self.mtype
                    or path.get("type") == "movie"
                    and self.mtype == MediaType.MOVIE
                    or path.get("type") == "tv"
                    and self.mtype == MediaType.TV
                    or path.get("type") == "anime"
                    and self.mtype == MediaType.ANIME
                ):
        ...
        if self.keyword:
            ...
            if _search_params:
                ...
                if self.category:
                    ...
                    for cat in cats:
                        if self.category.get("field"):
                            ...
                            params.update({f"cat{cat.get('id')}": 1})
                searchurl = (
                    self.domain
                    + torrentspath
                    + "?"
                ...
                # 变量字典
                inputs_dict = {"keyword": quote(search_word), "page": self.page or 0}
                # 无额外参数
        ...
        # 列表浏览
            ...
            # 变量字典
            inputs_dict = {"page": self.page or 0, "keyword": ""}
            # 有单独浏览路径
            if self.browse:
                if self.browse_list:
        ...
        if params:
            yield feapder.Request(url=searchurl, use_session=True, data=params, method="GET", retry_times=3)
        else:
            yield feapder.Request(url=searchurl, use_session=True, method="GET", retry_times=3)
    ...
    def get_title_default(self, torrent):
        ...
        self.torrents_info["title"] = self.__filter_text(self.torrents_info.get("title"), selector.get("filters"))
        ...
        if download_link:
            ...
            if not download_link.startswith("http") and not download_link.startswith("magnet"):
                self.torrents_info["enclosure"] = (
                    self.domain + download_link[1:] if download_link.startswith("/") else self.domain + download_link
                )
            else:
                self.torrents_info["enclosure"] = download_link

    def get_imdb_id(self, torrent):
        # imdbid
        ...
        self.torrents_info["imdbid"] = self.__filter_text(self.torrents_info.get("imdbid"), selector.get("filters"))
    ...
    def get_leechers(self, torrent):
        ...
        if item:
            ...
            self.torrents_info["peers"] = self.__filter_text(self.torrents_info.get("peers"), selector.get("filters"))
        else:
    ...
    def get_grabs(self, torrent):
        ...
        if item:
            ...
            self.torrents_info["grabs"] = self.__filter_text(self.torrents_info.get("grabs"), selector.get("filters"))
        else:
            self.torrents_info["grabs"] = 0

    def get_pub_date(self, torrent):
        # torrent pubdate
        ...
        self.torrents_info["pubdate"] = self.__filter_text(self.torrents_info.get("pubdate"), selector.get("filters"))
    ...
    def get_download_volume_factor(self, torrent):
    ...
    def get_upload_volume_factor(self, torrent):
        # uploadvolumefactor
        ...
        if "case" in selector:
            for uploadvolumefactorselector in list(selector.get("case", {}).keys()):
                ...
                if len(uploadvolumefactor) > 0:
                    self.torrents_info["uploadvolumefactor"] = selector.get("case", {}).get(uploadvolumefactorselector)
                    break
        elif "selector" in selector:
    ...
    def get_info(self, torrent):
        ...
        try:
            ...
            self.get_title_optional(torrent)
            ...
            self.get_imdb_id(torrent)
            self.get_download_volume_factor(torrent)
            self.get_upload_volume_factor(torrent)
            self.get_pub_date(torrent)
