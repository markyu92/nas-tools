...
(function (global, factory) {
  ...
  const IconifyIcon =
    (typeof window !== "undefined" && window.IconifyIcon) || null;
  ...
  const HistoryPage = {
    ...
    setup() {
      ...
      async function fetchData() {
        ...
        try {
          const res = await rc.get(
            "/api/plugin-framework/plugins/autosignin/data/signin_history.json",
          );
      ...
      const sortedDates = computed(() => {
        return Object.keys(historyData.value).sort((a, b) =>
          b.localeCompare(a),
        );
      });
      ...
      const summary = computed(() => {
        ...
        return {
          totalDays: Object.keys(historyData.value).length,
          totalSign,
          totalRetry,
        };
      });
        ...
        const stats = h(
          ...
          [
            renderStatCard(
              ...
              "签到天数",
              summary.value.totalDays,
              "primary",
            ),
            renderStatCard(
              "lucide:check-circle-2",
              "已签到",
              summary.value.totalSign,
              "success",
            ),
            renderStatCard(
              "lucide:alert-triangle",
              "需重试",
              summary.value.totalRetry,
              "warning",
            ),
          ],
        );
